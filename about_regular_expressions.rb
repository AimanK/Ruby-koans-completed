...
=begin
    When there are no values whatsoever in the input string?

    Just looked it up, it will never fail to match, even if it
    is checked against an empty string. As long as there is a
    string at all of course, it will match
...
=begin
  The repition operator algorithms will seek out the largest match possible,
  but will also always favor the left side of the string over the right,
  and only then go on to smaller and smaller matches until it finds a
  suitable one for the expression.
